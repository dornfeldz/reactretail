  ...
  return (
      <div className="border p-3 flex flex-col gap-3 rounded-xl">
          ...
          <div className="flex gap-4 items-center">
                  <button className="bg-yellow-300 w-7 h-7 rounded-full font-semibold" onClick={() => decreaseQuantity(item)}>-</button>
                  <p>Quantity: <span className="font-bold">{item.quantity}</span></p>
                  <button className="bg-yellow-300 w-7 h-7 rounded-full font-semibold" onClick={() => increaseQuantity(item)}>+</button>
              </div>
              <button className="bg-yellow-300 px-4 py-2 rounded-full font-semibold" onClick={() => setCartItems([...cartItems, item])}>Add to cart</button>
          </div>
      </div>
  );
}
